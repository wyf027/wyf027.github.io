...
function loadError() {
    $('#load-status').text('GitHub is temporarily unavailable or rate limited. Please reload, or open the GitHub profile.').show();
}

$(document).ready(function() {
    $.ajaxSetup({timeout: 20000});
    $(document).ajaxError(loadError);
    run();
});
...
var github_user_repos = function(username, callback, page_number, prev_data) {
    ...
    $.getJSON(url, function(repos) {
        if (!Array.isArray(repos.data)) { loadError(); return; }
...
var github_user_issues = function(username, callback) {
    $.getJSON('https://api.github.com/search/issues?q=type:pr+is:merged+is:public+author:' + username + '&sort=updated&order=desc&per_page=100&callback=?', function(response) {
        if (!response.data || !Array.isArray(response.data.items)) { loadError(); return; }
...
// This personal deployment is explicitly opted in by its owner.
var run = function() {
    var itemCount = 0, maxItems = 5, maxLanguages = 9;

    var res = github_user(username, function(data) {
        data = data.data;
        if (!data || data.login !== username) { loadError(); return; }
        ...
        if (data.blog !== undefined && data.blog !== null && data.blog !== '') {
            var website = addHttp + data.blog;
            if (/^https?:\/\//i.test(website)) view.website = website;
        }
                ...
                if (sorted.length > 0) {
                    ...
                    $.each(sorted, function(index, repo) {
                        ...
                        view = {
                            ...
                            homepage: /^https?:\/\//i.test(repo.info.homepage || '') ? repo.info.homepage : null,
                            username: username,
            ...
            success: function(response) {
                if (sorted.length > 0) {
                    ...
                    $.each(sorted, function(index, repo) {
                        ...
                        commitsUrl = repoUrl + '/pulls?q=is%3Apr+is%3Amerged+author%3A' + username;
                        view = {
    ...
    github_user_orgs(username, function(response) {
        ...
        if (!Array.isArray(response.data)) { loadError(); return; }
